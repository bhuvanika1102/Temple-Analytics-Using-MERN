...
const Query4 = () => {
  ...
  const handleQuerySubmit = async (values) => {
    try {
      const response = await fetch(`/api/query3?startDate=${values.d_start_date.format('YYYY')}&endDate=${values.d_end_date.format('YYYY')}&d_deity=${values.d_deity}`);
  ...
  return (
    <div className="query2-container">
      ...
      <div className="query4-sub">
        <h2>Query 3: Between 2years ,State-wise ,Deity-wise ,Number of Visitors</h2>
